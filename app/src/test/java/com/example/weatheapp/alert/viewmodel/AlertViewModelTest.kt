...
class AlertViewModelTest {
    ...
    @Test
    fun getAlertWeatherFromRoom_ReturnFlowableAlertWeatherEntityList() = runBlockingTest {

        var list = listOf<AlertWeatherEntity>()
        alertViewModel.getAlertWeatherFromRoom()
        alertViewModel.alert.test{
            this.awaitItem().apply {
                when(this){
                    is LocalAlertState.Success->  {list = this.alertWeather}
                    else->{}
                }
                assertEquals(alertList.size,list.size)
                assertEquals( alertList.toList().first(),list[0])
                assertEquals(list[0].id, alert1.id)
            }
        }}
        ...
        fun deleteFavWeatherFromRoom() = runBlockingTest {
            var list = listOf<AlertWeatherEntity>()
            alertViewModel.deleteALertWeatherFromRoom(alert3)
    ...
    @Test
    fun insertAlertWeatherIntoRoom() = runBlockingTest {
        var list = listOf<AlertWeatherEntity>()
        alertViewModel.insertALertWeatherFromRoom(alert5)
