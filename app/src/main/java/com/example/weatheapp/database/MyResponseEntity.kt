...
import com.example.weatheapp.model.*
import com.google.gson.Gson
import com.google.gson.reflect.TypeToken

@Entity(tableName = "home_table")
data class MyResponseEntity(@PrimaryKey val id: Int = 1,val countryName:String?,
                      var lat:Double, var lon:Double, var current: Current?,
                      var hourly:List<Hourly>?, var daily:List<Daily>?,var alert: List<Alert>?) {
...
/*    @TypeConverter
    ...
    @TypeConverter
    fun fromStringToAlert(alert: String?): Array<Alert>? {
        return if (alert != null) {
            Gson().fromJson(alert, Array<Alert>::class.java)
        } else {
            null
        }
    }*/
